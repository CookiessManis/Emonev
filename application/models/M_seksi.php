...
class M_seksi extends CI_Model
{
  ...
  {
      $this->db->select('belanja.username, user.nama, SUM(belanja.realisasi) as total_realisasi,SUM(belanja.target) as total_target, SUM(belanja.realisasi_fisik) as trealisasi_fisik, SUM(belanja.target_fisik) as ttarger_fisik');
   ...
   public function get3($id_anggaran)
  {
    ...
    $this->db->where('anggaran.id_anggaran', $id_anggaran);
      ...
      // Add the condition for id_bulan
           $this->db->where('(belanja.id_bulan > 6 AND belanja.id_bulan < 10)');
